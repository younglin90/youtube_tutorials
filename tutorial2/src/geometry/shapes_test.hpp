...
namespace SimulMan {
    ...
    public:
        ...
        virtual vec3 closest() const = 0;
        virtual bool is_overlap() const = 0;
    };
    ...
    // 1d
    template<typename InsiderStrategy
        , typename ClosesterStrategy
        , typename OverlaperStrategy>
    struct Point : public Shape {
        vec3 p{};
        
        Point() :
            insider_(InsiderStrategy{})
            , closester_(ClosesterStrategy{})
            , overlaper_(OverlaperStrategy{})
        {

        }

        bool is_inside() const override {
            return insider_(*this);
        }
        vec3 closest() const override {
            return closester_(*this);
        }
        bool is_overlap() const override {
            return overlaper_(*this);
        }
    private:
        InsiderStrategy insider_;
    ...
    public:
        ...
        bool operator()(const vec3& p, const Sphere& s) const { return is_inside(p, s); }
    ...
    public:
        ...
        vec3 operator()(const vec3& p, const Sphere& s) const { return closest(p, s); }
