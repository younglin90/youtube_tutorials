	...
	RigidBody sphere(
		...
		for (int i = 0; i <= segments; ++i) {
			...
			for (int j = 0; j <= segments; ++j) {
				...
				float x = center[0] + radius * std::sin(phi) * std::cos(theta);
				float y = center[1] + radius * std::sin(phi) * std::sin(theta);
				float z = center[2] + radius * std::cos(phi);
				rigid.pos.push_back(Eigen::Vector3d(x, y, z));
			}
		}
		rigid.pos.push_back(Eigen::Vector3d(0, 0, 0));
		uint32_t vc = rigid.pos.size() - 1;
		...
		for (int i = 0; i < segments; ++i) {
			for (int j = 0; j < segments; ++j) {
				...
				rigid.f2v.push_back({ v1,v3,v2 });
				rigid.f2v.push_back({ v2,v3,v4 });
